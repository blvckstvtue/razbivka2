...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max) 
{
	...
	{
		SetFailState("[CASEOPENER] Error loading plugin: Only for CS:Source");
...
public Plugin myinfo = 
{
	name = "Case Opener",
	author = "TRASHLORD",
	description = "CSGO Case with rewards",
	version = "1.5.0",
	url = "https://liondevs.zone.id/"
}
...
public Action Command_Case(int client, int args) 
{
	...
	{
		if(bCaseMessages) 
			CPrintToChat(client, "%t%t", "prefix", "be_alive");
...
public Action CommandResetCounter(int client, int args) 
{
	...
	{
		if(bCaseMessages) 
			CPrintToChat(client, "%t%t", "prefix", "not_works");
...
public Action Hook_ModelStartTouch(int iEntity, int activator) 
{
	if(0 < activator <= MaxClients)
	{
		if(iEntCaseData[activator][1] == iEntity && iEntity > MaxClients)
		{
			...
			switch(iReward[activator]) 
			{
				case 0: 
				{
					...
					{
						if(bPrintAll) 
							CPrintToChatAll("%t%t", "prefix", "received_credits_all", activator, iEntCaseData[activator][4]);
						else 
							CPrintToChat(activator, "%t%t", "prefix", "received_credits", iEntCaseData[activator][4]);
					}
				...
				case 1: 
				{
					...
					{	
						...
						{
							if(bPrintAll) 
								CPrintToChatAll("%t%t", "prefix", "received_exp_all", activator, iEntCaseData[activator][4]);
							else 
								CPrintToChat(activator, "%t%t", "prefix", "received_exp", iEntCaseData[activator][4]);
						}
					...
					if(bGiveVIP) 
					{
						if(!VIP_IsClientVIP(activator)) 
						{
							...
							if(bCaseMessages) 
							{
								...
								{
									if(!kv.GetNum("time")) 
										CPrintToChatAll("%t%t", "prefix", "got_vip_all_forever", activator, buffer);
									else 
										CPrintToChatAll("%t%t", "prefix", "got_vip_all", activator, buffer, kv.GetNum("time"));
								}
							}
						...
						else 
						{
							...
							{
								if(bPrintAll) 
									CPrintToChatAll("%t%t", "prefix", "nothing", activator);
								else 
									CPrintToChat(activator, "%t%t", "prefix", "already_has_vip");
							}
			...
			EmitSoundToClient(activator, "roulette/claime.mp3", SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, NULL_VECTOR);
			if(bStartCounter)
			{
		...
		else if(bCaseMessages) 
		{
			if(!bWarn[activator]) 
			{
				CPrintToChat(activator, "%t%t", "prefix", "not_your_case");
...
public void SQLResetCounterCB(Database db, DBResultSet result, const char[] error, int client)
{
	if(result && !error[0])
	{
		if(result.HasResults && result.RowCount) 
		{
			...
			if(bCaseMessages) 
				CPrintToChat(client, "%t%t", "prefix", "counter_reseted");
		}
	}
	else 
			...
			if(result.RowCount)
			{
					...
					iEntCaseData[client][4] == -1 ) 
				{
					if(bCaseAccess) 
					{
						...
						{
							if(bCaseMessages) 
								CPrintToChat(client, "%t%t", "prefix", "not_admin");
					...
					if(!IsFakeClient(client)) 
					{
						...
						if(TR_DidHit(hTrace) && hTrace != INVALID_HANDLE) 
						{
							...
							{
								if(bCaseMessages) 
									CPrintToChat(client, "%t%t", "prefix", "same_level_case");
							}
							else if(GetVectorDistance(fClientOrigin, fEndOfTrace) > float(iMaxPositionValue * 100) && bMaxPosition) 
							{
								if(bCaseMessages) 
									CPrintToChat(client, "%t%t", "prefix", "too_longer", iMaxPositionValue);
				...
				{
					if(bCaseMessages) CPrintToChat(client, "%t%t", "prefix", "existing_case");
					EmitSoundToClient(client, "buttons/blip1.wav", SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, NULL_VECTOR);
				}
			}
			else
...
public void SQLTCheckStatusForTime(Database db, DBResultSet result, const char[] error, int client)
{
	if(result && !error[0])
	{
		if(result.HasResults && result.RowCount && result.FetchRow())
		{
			...
			if(time >= 0) 
				CPrintToChat(client, "%t%t", "prefix", "wait_next_case", time/3600/24, time/3600%24, time/60%60, time%60);
			EmitSoundToClient(client, "buttons/blip1.wav", SOUND_FROM_PLAYER, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, NULL_VECTOR);
...
public Action Scrolling(Handle hNewTimer, int client) 
{
	...
	if(bCaseOpeningSound) 
		EmitSoundToAll("roulette/rot.mp3", iEntCaseData[client][0], SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, fPos);
...
public Action SoundOpen(Handle hNewTimer, int client) 
{
	...
	GetEntPropVector(iEntCaseData[client][0], Prop_Data, "m_vecAbsOrigin", fPos);
	if(bCaseOpeningSound) 
		EmitSoundToAll("buttons/blip1.wav", iEntCaseData[client][0], SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, fPos);
...
float[] SpawnCase(int iClient, float fPos[3], float fAng[3]) 
{
	...
	EmitSoundToAll("roulette/rot.mp3", iEntCaseData[iClient][0], SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, fPos);
